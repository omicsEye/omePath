...
enrichment_score <- function() {
  ...
  total <-
    sfsmisc::integrate.xy(d$x, d$a) + sfsmisc::integrate.xy(d$x, d$b)
  ...
  FDensity = stats::approxfun(stats::density(df$weight[df$sex == "F"], from =
                                               40, to = 80))
  MDensity = stats::approxfun(stats::density(df$weight[df$sex == "M"], from =
                                               40, to = 80))
  ...
  FminusM = function(x) {
    FDensity(x) - MDensity(x)
  }
  Intersect = stats::uniroot(FminusM, c(40, 80))$root
  graphics::points(Intersect,
                   FDensity(Intersect),
                   pch = 20,
                   col = "red")
